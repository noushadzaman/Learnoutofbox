...
export async function GET(request) {
  try {
    ...
    completionInfo.instructorDesignation !== undefined &&
    ...
    // pattern
    const patternUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/Certificate_template.png`;

    const patternBytes = await fetch(patternUrl).then((res) =>
      res.arrayBuffer()
    );
    const pattern = await pdfDoc.embedPng(patternBytes);
    console.log(pattern);
